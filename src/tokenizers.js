...
class AutoTokenizer {
	static async fromPretrained(filePath) {
		const tokenizerFile = readFileSync(filePath, 'utf8');
		const tokenizer = Tokenizer.fromConfig(JSON.parse(tokenizerFile));
		return tokenizer;
	}
}
